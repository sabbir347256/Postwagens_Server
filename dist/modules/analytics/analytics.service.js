...
const getAnalyticsSummary = async () => {
    const totalUsers = await user_model_1.default.countDocuments({ isActive: user_interface_1.IsActive.ACTIVE });
    const listingStatsArr = await listing_model_1.default.aggregate([
        {
            $group: {
                _id: null,
    ...
    const { totalViews: totalListingViews, totalInquiries: totalListingInquiries } = listingStats;
    const engagementRate = totalListingViews > 0
    ...
    const newUsersLastMonth = await user_model_1.default.countDocuments({
    ...
    const monthlyListingStatsArr = await listing_model_1.default.aggregate([
        {
            $match: {
    ...
    const thisMonthData = monthlyListingStatsArr.find(d => d._id.year === thisMonthId.year && d._id.month === thisMonthId.month) || { monthlyViews: 0, monthlyInquiries: 0 };
    const lastMonthData = monthlyListingStatsArr.find(d => d._id.year === lastMonthId.year && d._id.month === lastMonthId.month) || { monthlyViews: 0, monthlyInquiries: 0 };
    // Listing view growth
    const listingViewGrowth = lastMonthData.monthlyViews > 0
        ...
        : thisMonthData.monthlyViews > 0
            ...
            : 0;
    // Engagement growth
    const engagementRateThisMonth = thisMonthData.monthlyViews > 0
        ? (thisMonthData.monthlyInquiries / thisMonthData.monthlyViews) * 100
        : 0;
    const engagementRateLastMonth = lastMonthData.monthlyViews > 0
        ? (lastMonthData.monthlyInquiries / lastMonthData.monthlyViews) * 100
        : 0;
    const engagementGrowth = engagementRateLastMonth > 0
...
const getUserGrowth = async (range) => {
    const now = new Date();
    const data = [];
    if (range === 'monthly') {
        for (let i = 5; i >= 0; i--) {
            ...
            const currentMonthCount = await user_model_1.default.countDocuments({
                createdAt: { $gte: startDate, $lt: endDate },
            });
            const previousMonthCount = await user_model_1.default.countDocuments({
        ...
        for (let i = 3; i >= 0; i--) {
            ...
            const weekEnd = new Date(now.getFullYear(), now.getMonth(), now.getDate() - now.getDay() - i * 7 + 7);
            const prevWeekStart = new Date(now.getFullYear(), now.getMonth(), now.getDate() - now.getDay() - (i + 1) * 7);
            const prevWeekEnd = new Date(now.getFullYear(), now.getMonth(), now.getDate() - now.getDay() - (i + 1) * 7 + 7);
            const currentWeekCount = await user_model_1.default.countDocuments({
                createdAt: { $gte: weekStart, $lt: weekEnd },
            });
            const prevWeekCount = await user_model_1.default.countDocuments({
...
const getTopListings = async () => {
    const listings = await listing_model_1.default.find()
        .sort({ viewCount: -1 })
        .limit(10)
